...
import { deleteVanCarById } from 'apiSdk/van-cars';
...
function BookingViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BookingInterface>(
    ...
    () =>
      getBookingById(id, {
        relations: ['booking_manager', 'company', 'van_car'],
      }),
  );

  const van_carHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
            ...
            <Box>
              {hasAccess('booking_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Flex alignItems="center" mb={4}>
                  ...
                  <Text fontSize="md" as="span" ml={3}>
                    <Link as={NextLink} href={`/booking-managers/view/${data?.booking_manager?.id}`}>
                      {data?.booking_manager?.id}
                    </Link>
                  </Text>
                </Flex>
              )}
              ...
              {hasAccess('van_car', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/van-cars/create?booking_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.van_car?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/van-cars/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            <Td>
